                ...
                <div class="box">
                    <div class="box-body">
                        <form class="form-horizontal" enctype='multipart/form-data'
                            action="{{ url('admin/properties') }}" method="GET" accept-charset="UTF-8">
                            {{ csrf_field() }}
                                        ...
                                        <div class="col-md-3 col-sm-3 col-xs-12">
                                            ...
                                            <select class="form-control" name="space_type" id="space_type">
                                                ...
                                                <option value="{{$data->id}}"
                                                    {{$data->id == $allSpaceType ? "selected": ''}}>{{$data->name}}
                                                </option>
                                                @endforeach
            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        ...
                        <div class="pull-right"><a class="btn btn-success" href="{{ url('admin/add-properties') }}">Add
                                Properties</a></div>
                        @endif
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        <div class="table-responsive">
                            {!! $dataTable->table(['class' => 'table table-striped table-hover dt-responsive', 'width'
                            => '100%', 'cellspacing' => '0']) !!}
                        </div>
                    </div>
                </div>
...
$(function() {
    ...
    $(document).on("click", "#csv", function(event) {
        ...
        window.location = "properties/property_list_csv?to=" + to + "&from=" + from + "&space_type=" +
            space_type + "&status=" + status;
    });
    //pdf convert
    $(document).on("click", "#pdf", function(event) {
        ...
        var to = $('#endDate').val();
        var from = $('#startDate').val();
        window.location = "properties/property_list_pdf?to=" + to + "&from=" + from + "&space_type=" +
            space_type + "&status=" + status;
    });
    //reload Datatable
